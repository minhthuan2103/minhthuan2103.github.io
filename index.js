...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function parseNgayYl(ngayYlStr) {
        if (!ngayYlStr || ngayYlStr.length !== 12)
            return null;
    ...
    async function parseExcelData(file) {
        console.log('Inside parseExcelData. Checking window.XLSX:', typeof window.XLSX, window.XLSX);
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (event) => {
                try {
                    ...
                    if (!parsedSuccessfully && (isExcelFileByMimeOrExt || isCsvOrTxtFileByMimeOrExt))
                        displayMessage(`Không thể trích xuất dữ liệu nhân viên từ ${file.name}.`, 'info-message', true);
                        ...
                        displayMessage(`Không tìm thấy dữ liệu nhân viên hợp lệ trong ${file.name}.`, 'info-message', true);
                    resolve(newEmployeeData);
                }
                catch (e) {
            ...
            const canUseXLSXLib = typeof window.XLSX !== 'undefined';
            ...
            if (canUseXLSXLib && isExcelFile)
                ...
                if (isExcelFile && !canUseXLSXLib)
                    message = `Thư viện XLSX cần để xử lý "${file.name}". Hiện tại chỉ hỗ trợ .csv hoặc .txt.`;
    ...
    async function handleFileProcessing() {
        ...
        try {
            ...
            for (let i = 0; i < fileContents.length; i++) {
                ...
                if (!fileContent)
                    throw new Error(`Nội dung tệp ${fileName} trống.`);
    ...
    function generateTimesheet(xmlDocs, currentEmployeeData) {
            ...
            parentTagNames.forEach(parentTagName => {
                ...
                for (const el of Array.from(elements)) {
                    ...
                    if (maBacSiEl?.textContent?.trim() && ngayYlEl?.textContent?.trim()) {
                        ...
                        if (dateTime)
                            entries.push({ id: maBacSiEl.textContent.trim(), dateTime, isNthEntry: false });
                    }
        ...
        for (const entry of allEntries) {
            ...
            if (timeValue >= 700 && timeValue < 1130) {
                ...
                if (isNthEntry)
                    timesheetData[id][day].S_isNth = true;
                ...
                if (isNthEntry)
                    timesheetData[id][day].C_isNth = true;
    ...
    function populateTableBody(tbody, timesheetData, year, month, // 0-indexed
        ...
        uniqueIdsForTable.forEach(id => {
            ...
            row.addEventListener('dragstart', (event) => {
                ...
                tbody.querySelectorAll('tr').forEach(tr => {
                    if (tr !== currentTargetRow)
                        tr.classList.add('drop-candidate');
                });
            });
            ...
            row.addEventListener('dragover', (event) => {
                ...
                if (!draggedItemId || targetRow.dataset.id === draggedItemId)
                    return;
            ...
            row.addEventListener('drop', (event) => {
                ...
                if (draggedItemId && droppedOnItemId && draggedItemId !== droppedOnItemId) {
                    ...
                    if (draggedIndex !== -1 && droppedOnIndex !== -1) {
                        ...
                        console.log('Inside drop event. Checking window.XLSX before re-setupDownloadButton:', typeof window.XLSX, window.XLSX);
                        if (downloadButton && typeof window.XLSX !== 'undefined') {
    ...
    function displayTimesheetTable(timesheetData, year, month, // 0-indexed
    daysInMonth, currentEmployeeData) {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if ([0, 6].includes(getDayOfWeek(year, month, day)))
        ...
        console.log('Inside displayTimesheetTable. Checking window.XLSX before download button setup:', typeof window.XLSX, window.XLSX);
        if (uniqueIdsForTable.length > 0 && typeof window.XLSX !== 'undefined') {
        ...
        else if (uniqueIdsForTable.length > 0 && typeof window.XLSX === 'undefined') {
            displayMessage('Lỗi: Không thể tạo nút tải xuống Excel do thư viện XLSX chưa sẵn sàng.', 'error-message', true);
        }
    }
    function setupDownloadButton(button, currentOrderedIds, timesheetData, year, month, // 0-indexed
    daysInMonth, currentEmployeeData) {
        ...
        button.onclick = () => {
            ...
            try {
                ...
                const worksheet = window.XLSX.utils.aoa_to_sheet(excelSheetData);
                worksheet[window.XLSX.utils.encode_cell({ r: 0, c: 0 })].s = baseHeaderStyle;
                for (let c = 1; c <= daysInMonth; c++) {
                    ...
                    worksheet[window.XLSX.utils.encode_cell({ r: 0, c: c })].s = style;
                }
                for (let c = 1; c <= daysInMonth; c++) {
                    const dayOfWeek = getDayOfWeek(year, month, c);
                    ...
                    worksheet[window.XLSX.utils.encode_cell({ r: 1, c: c })].s = style;
                }
                currentOrderedIds.forEach((id, rowIndex) => {
                    const r = rowIndex + 2;
                    let idIsNthSource = false;
                    for (let dayIdx = 1; dayIdx <= daysInMonth; dayIdx++)
                        if (timesheetData[id]?.[dayIdx] && (timesheetData[id][dayIdx].S_isNth || timesheetData[id][dayIdx].C_isNth)) {
                            idIsNthSource = true;
                            break;
                        }
                    worksheet[window.XLSX.utils.encode_cell({ r: r, c: 0 })].s = idIsNthSource ? baseIdNameNthStyle : baseIdNameStyle;
                    for (let day = 1; day <= daysInMonth; day++) {
                        const excelColIdx = day;
                        const isWeekend = [0, 6].includes(getDayOfWeek(year, month, day));
                        const cellAddress = window.XLSX.utils.encode_cell({ r: r, c: excelColIdx });
                        const cellObj = worksheet[cellAddress];
                        const markVal = cellObj ? cellObj.v : '';
                        let cellIsNthDerived = false;
                        if (timesheetData[id]?.[day]) {
                            const att = timesheetData[id][day];
                            if ((markVal === '+' && (att.S_isNth || att.C_isNth)) || (markVal === 'S' && att.S_isNth) || (markVal === 'C' && att.C_isNth))
                                cellIsNthDerived = true;
                        }
                        const initialCellStyle = markVal ?
                            (cellIsNthDerived ? baseMarkNthStyle : baseMarkStyle)
                            : baseEmptyCellStyle;
                        let currentCellStyle = { ...initialCellStyle }; // Create a mutable copy
                        if (isWeekend) {
                        ...
                        if (cellObj) {
                            ...
                            window.XLSX.utils.sheet_add_aoa(worksheet, [['']], { origin: { r: r, c: excelColIdx } });
                            worksheet[cellAddress].s = currentCellStyle;
                        }
                    }
                });
